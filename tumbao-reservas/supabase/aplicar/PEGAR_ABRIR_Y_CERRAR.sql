-- =====================================================================
-- TUMBAO · ABRIR LA CAJA, Y QUE CERRARLA SIGNIFIQUE ALGO
-- Pegar completo en el editor SQL de Supabase y darle Run.
--
-- SE PUEDE APLICAR CON EL DÍA EMPEZADO
-- Hoy ya se usó el sistema: hay movimientos y no hay apertura. Nada de
-- esto lo exige. Un día sin apertura sigue funcionando igual que hasta
-- ahora y se puede abrir a media tarde si se quiere. Está probado
-- explícitamente: es el primer caso de la prueba de humo.
--
-- QUÉ TRAE
--
--  1. ABRIR LA CAJA. Se cuentan los billetes al empezar y la base del
--     día pasa a ser lo CONTADO, no lo que decía el papel de anoche. Si
--     no cuadran, la diferencia queda anotada en la apertura. Eso parte
--     un descuadre en dos —lo de anoche y lo de hoy— y cada mitad tiene
--     dueño. Hasta ahora un faltante aparecía a las nueve, mezclado con
--     las ventas del día, y ya no se sabía de qué turno era.
--
--  2. QUE CERRADO SEA CERRADO. Se tapan dos agujeros reales:
--       · caja_registrar NO miraba si el día estaba cerrado. Se podía
--         cerrar a las 9, vender a las 9:05, y el papel impreso quedaba
--         mintiendo. (caja_anular sí lo miraba: se podía deshacer pero
--         no agregar, al revés de como debía ser.)
--       · caja_cerrar sobrescribía el cierre en silencio. Ahora rehacerlo
--         se pide a propósito, con motivo, y queda contado.
--
--  3. LO QUE NECESITA LA TIRILLA. caja_del_dia devuelve el resumen por
--     concepto, quién abrió y cerró, y a qué hora.
--
-- SEGURIDAD
-- No borra ninguna fila. La tabla nueva arranca vacía. Se puede correr
-- dos veces. Si algo saliera mal, la caja sigue funcionando como hoy.
-- =====================================================================
