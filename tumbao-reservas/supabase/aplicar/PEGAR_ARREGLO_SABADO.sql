-- ═══════════════════════════════════════════════════════════════
--
--   TUMBAO — ARREGLO IMPORTANTE
--
--   Pégalo en el SQL Editor de Supabase y dale Run. Son 3 líneas.
--
--   ¿Qué arregla? Un bug que iba a romper la importación de la noche
--   la primera vez que un miembro reservara un sábado.
--
--   La reserva del miembro guarda un vínculo a su membresía. Y la
--   importación nocturna BORRA la tabla de membresías entera para
--   recargarla. Con el vínculo puesto, ese borrado fallaba y la
--   importación abortaba: los cupos se quedaban congelados en los del
--   último día bueno, y nadie se enteraba.
--
--   Reservar el sábado es justo lo que los miembros tienen que hacer,
--   así que iba a pasar el primer sábado.
--
...
-- ═══════════════════════════════════════════════════════════════
--   Comprobación — tiene que decir ON DELETE SET NULL
-- ═══════════════════════════════════════════════════════════════
select 'Arreglado' as estado, pg_get_constraintdef(oid) as regla
  from pg_constraint
 where conrelid = 'reservas'::regclass
   and conname = 'reservas_membresia_id_fkey';
