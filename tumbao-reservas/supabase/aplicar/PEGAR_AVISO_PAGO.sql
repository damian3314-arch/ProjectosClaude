-- ═══════════════════════════════════════════════════════════════
--
--   TUMBAO — AVISO DE PAGO
--
--   Pégalo en el SQL Editor de Supabase y dale Run. Corto.
--
--   ¿Qué agrega? Cuando alguien dice "ya pagué", ahora también
--   declara la HORA de su transferencia, y si pagó otra persona,
--   a nombre de quién sale la cuenta.
--
--   Por qué importa: el pago se buscaba en una ventana de 3 HORAS
--   alrededor de cuando la persona EMPEZÓ a reservar. Ahí caben
--   varias pagando $15.000, y para desempatar se usaba el nombre.
--   Pero muchas veces paga la mamá o la pareja, así que el nombre
--   no sirve justo cuando más se necesita.
--
--   Con la hora del comprobante la ventana pasa a ±15 minutos.
--
--   NO se guarda ninguna imagen. El comprobante se lee en el
--   celular de quien paga y se descarta ahí mismo.
--
...
--   Comprobación
-- ═══════════════════════════════════════════════════════════════
select 'Aviso de pago listo' as estado,
       (select count(*) from information_schema.columns
         where table_name = 'reservas'
           and column_name in ('pagado_en','pagador_nombre','referencia_pago','comprobante_qr'))
         as columnas_nuevas_de_4,
       (select count(*) from pg_proc p join pg_namespace n on n.oid = p.pronamespace
         where n.nspname = 'public' and p.proname = 'registrar_aviso_pago')
         as funcion_creada;
